...
const CONTRACTS = [
  ...
  { id: 'assignment-event-raven-bridge', layer: 'api', source: 'app/api/realm-demo/actions/route.js', signals: ['repositoryRealmsSurface(body?.action)', 'executeRepositoryRealmsAction', 'await emitEvent(result.resource', 'if (!result.idempotent)'] },
  ...
  { id: 'realtime-task-and-approval-domains', layer: 'sync', source: 'lib/realm-change-feed.js', signals: ["tasks: ['operations', 'guild', 'campaigns', 'command', 'rewards', 'chronicle']", "approvals: ['command', 'notifications', 'chronicle']"] },
