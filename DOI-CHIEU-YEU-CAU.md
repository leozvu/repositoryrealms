...
> Cập nhật 12/07/2026 — sau v3.2. Ký hiệu: ✅ đã có · 🟡 có một phần · 🔨 nên xây tiếp · 🔌 nên tích hợp dịch vụ ngoài (không tự xây) · ⛔ không phù hợp agency 10–25 người
...
| V | **Customer Success** (ticket, SLA, NPS, churn) | ✅ | Ticket + SLA tự tính + cảnh báo vỡ SLA (v2.4) · **NPS khảo sát + điểm trong Analytics (v2.5)** · churn prediction rule-based trong AI Summary. 🔨: CSAT riêng. 🔌 Live chat: Tawk.to (miễn phí) |
...
| VII | **HR** | ✅ | Hồ sơ, chấm công ngày, nghỉ phép + duyệt, **bảng lương BHXH/TNCN**, tuyển dụng kanban, nhóm/org + **KPI/OKR quý (v2.5)**. 🔨: đào tạo |
...
| XVI | **Calendar** | ✅ | Lịch gộp 6 loại sự kiện. 🔌 đồng bộ Google Calendar ở v3.3 |
| XVII | **Workflow Automation** | 🟡 | Máy phê duyệt đa bước + hóa đơn định kỳ + chặn task theo phụ thuộc = automation lõi. 🔨 v3.3: rule builder IF/THEN. 🔌 n8n/Zapier khi có API mở |
...
| XXI | **API** | 🟡 | REST nội bộ đã chuẩn hóa; 🔨 v3.3: mở API key + webhook (sau khi deploy) |
...
| XXIV | **Analytics** (LTV, CAC, MRR, cohort) | ✅ | **Trang Analytics (v2.5): MRR/ARR, LTV, CAC + LTV/CAC, khách mới vs quay lại, NPS**. 🔨: cohort matrix đầy đủ |
...
2. **v3.3**: API key + webhook mở · đồng bộ Google Calendar · rule builder IF/THEN · CSAT + cohort matrix
3. **Tích hợp**: kéo số liệu Facebook Ads/ClickUp về báo cáo (đã có MCP trong Claude) · e-sign · đối soát ngân hàng
