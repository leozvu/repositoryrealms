...
export async function updateLocalCeoDirectoryProfile(db, user, input = {}, now = new Date()) {
  ...
  await db.$transaction(async (tx) => {
    const local = await tx.user.findUnique({ where: { id: user.id }, select: { id: true, name: true, title: true, status: true } });
    if (!local || local.status !== 'active') throw new CeoMessagingError('Employee profile is unavailable.', 404, 'ceo_messaging_employee_unavailable');
    await tx.ceoEntityDirectoryProfile.upsert({
      ...
      update: { sharedWithCeoPortal: shared, sharePresence: presence, displayName: local.name, title: local.title, consentedAt: shared ? now : undefined, revokedAt: shared ? null : now },
    });
    await tx.auditLog.create({ data: { userId: user.id, userName: user.name, action: shared ? 'ceo_directory_share_enabled' : 'ceo_directory_share_revoked', entity: 'ceo_directory_profile', refId: user.id, detail: `policy=${CEO_MESSAGING_POLICY.version}; presence=${presence}` } });
