...
import { transactionAmountVnd } from './money.js';
import { financialConversionIssues, invoiceReceivableVnd } from './financial-reporting.js';
...
export async function buildInsights() {
  const [txs, invoices, leads, clients, projects, tasks, timeLogs, users, contracts, activities, tickets, bills, assets, settingRow, shipments, areaCodes, violations] = await Promise.all([
    ...
    prisma.contract.findMany(), prisma.activity.findMany(), prisma.ticket.findMany(), prisma.vendorBill.findMany(), prisma.asset.findMany(),
  ...
  const conversionIssues = financialConversionIssues({ transactions: txs, invoices });
  const financeValid = conversionIssues.length === 0;
  if (!financeValid) add('bad', `${conversionIssues.length} bản ghi tài chính có số tiền hoặc tỷ giá không hợp lệ; chưa thể tổng hợp doanh thu, công nợ và burn rate VND.`, FIN, 'finance');

  /* ---- Doanh thu so tháng trước (cùng số ngày) ---- */
  const day = +today.slice(8, 10);
  const revTo = (k, d) => !financeValid ? 0 : txs.filter(t => t.type === 'income' && t.date.startsWith(k) && +t.date.slice(8, 10) <= d).reduce((s, t) => s + transactionAmountVnd(t), 0);
  ...
  const overdue = invoices.filter(v => !['paid', 'draft', 'void'].includes(v.status) && v.dueDate && v.dueDate < today && grand(v) - paidOf(v) > 0);
  if (overdue.length && financeValid) {
    const total = overdue.reduce((s, v) => s + invoiceReceivableVnd(v), 0);
  ...
  const burn = !financeValid ? 0 : Math.round(last3.reduce((s, k) => s + txs.filter(t => t.type === 'expense' && t.date.startsWith(k)).reduce((x, t) => x + transactionAmountVnd(t), 0), 0) / 3);
  const avgIn = !financeValid ? 0 : Math.round(last3.reduce((s, k) => s + txs.filter(t => t.type === 'income' && t.date.startsWith(k)).reduce((x, t) => x + transactionAmountVnd(t), 0), 0) / 3);
  ...
  /* ---- v3.30: Tài sản/license sắp phải gia hạn (đừng để domain/phần mềm hết hạn) ---- */
  const assetSoon = assets.filter(a => a.renewAt && a.renewAt >= today && a.renewAt <= addDays(today, 30) && a.status !== 'sold');
  if (assetSoon.length) add('warn', `${assetSoon.length} tài sản/license cần gia hạn trong 30 ngày: ${assetSoon.map(a => a.name).slice(0, 3).join(', ')}${assetSoon.length > 3 ? '…' : ''} — gia hạn trước khi hết hạn.`, ['DIRECTOR', 'ACCOUNTANT'], 'assets');
  ...
  /* ---- v3.30: Chất lượng dữ liệu — khách hàng trùng SĐT/email (nhập trùng, phân tán lịch sử) ---- */
  const normPhone = p => String(p || '').replace(/\D/g, '');
  const dupGroups = {};
  for (const c of clients) {
    const keys = [normPhone(c.phone) && 'p:' + normPhone(c.phone), c.email && 'e:' + c.email.trim().toLowerCase()].filter(Boolean);
    for (const k of keys) (dupGroups[k] ||= []).push(c.name);
  }
  const dups = Object.values(dupGroups).filter(g => g.length > 1);
  if (dups.length) {
    const sample = [...new Set(dups.flat())].slice(0, 3).join(', ');
    add('warn', `${dups.length} nhóm khách hàng trùng SĐT/email (${sample}${dups.flat().length > 3 ? '…' : ''}) — nên gộp để không phân tán lịch sử giao dịch.`, SALES, 'clients');
  }
