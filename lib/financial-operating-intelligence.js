...
import { invoiceTotalVnd, invoiceCollectedVnd, invoiceReceivableVnd } from './financial-reporting.js';
...
function invoiceSnapshot(invoice) {
  ...
  const document = { ...invoice, items: JSON.stringify(items), payments: JSON.stringify(payments) };
  const total = invoiceTotalVnd(document);
  const paid = invoiceCollectedVnd(document);
  const remaining = invoiceReceivableVnd(document);
...
export function buildFinancialOperatingIntelligence({
  ...
  const queue = [];
  const invoiceRows = invoices.filter(invoice => !['draft', 'cancelled', 'void'].includes(invoice.status)).map(invoiceSnapshot);
  const validInvoices = invoiceRows.filter((invoice) => invoice.valid);
  ...
  for (const transaction of transactions) {
    const value = transactionAmountVnd(transaction);
  ...
  return Object.freeze({
    ...
    provenance: Object.freeze({
      ...
      collectionFx: 'receipt_book_rate_snapshot_or_legacy_invoice_rate',
      receivableFx: 'unpaid_native_amount_at_invoice_book_rate',
