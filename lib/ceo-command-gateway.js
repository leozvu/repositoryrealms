...
export const CEO_COMMAND_DEFINITIONS = Object.freeze([
  ...
  Object.freeze({
    action: 'task.adjust',
    scope: 'command.task.adjust',
    capability: 'delivery',
    resource: 'tasks',
    label: 'Adjust task assignment',
  }),
  Object.freeze({
  ...
  Object.freeze({
    ...
    scope: 'command.project.status.request',
    capability: 'delivery',
    resource: 'tasks',
  ...
  Object.freeze({
    action: 'group_workforce.request',
    scope: 'command.group-workforce.request',
    capability: 'people',
    resource: 'approvals',
    label: 'Request a group workforce assignment',
  }),
  Object.freeze({
    action: 'incident.acknowledge',
    scope: 'command.incident.acknowledge',
    capability: 'support',
    resource: 'tickets',
...
function normalizeTaskAdjust(payload) {
  exactObject(payload, ['taskId', 'expectedUpdatedAt', 'assigneeEmail', 'dueDate', 'priority', 'estHours'], 'payload');
  const supplied = ['assigneeEmail', 'dueDate', 'priority', 'estHours']
    .filter((field) => Object.hasOwn(payload, field));
  if (!supplied.length) fail('At least one task field must be adjusted.', 400, 'ceo_command_task_adjust_empty');
  return {
    taskId: token(payload.taskId, 'task_id'),
    expectedUpdatedAt: timestamp(payload.expectedUpdatedAt, 'expected_updated_at'),
    ...(Object.hasOwn(payload, 'assigneeEmail') ? { assigneeEmail: email(payload.assigneeEmail, 'assignee_email') } : {}),
    ...(Object.hasOwn(payload, 'dueDate') ? { dueDate: day(payload.dueDate, false) } : {}),
    ...(Object.hasOwn(payload, 'priority') ? { priority: enumValue(payload.priority, PRIORITY, 'priority') } : {}),
    ...(Object.hasOwn(payload, 'estHours') ? { estHours: finite(payload.estHours, 'estimated_hours', 0, 1_000) } : {}),
  };
}
...
function normalizeProjectStatusRequest(payload) {
  exactObject(payload, ['projectId', 'message', 'targetEmail', 'dueDate', 'priority'], 'payload');
  return {
    projectId: token(payload.projectId, 'project_id'),
    message: text(payload.message, 'message', 1, 800, { multiline: true }),
    targetEmail: email(payload.targetEmail, 'target_email'),
    dueDate: day(payload.dueDate),
    priority: enumValue(payload.priority, new Set(['medium', 'high', 'urgent']), 'priority', 'medium'),
...
function normalizeGroupWorkforceRequest(payload) {
  exactObject(payload, [
    'requestingEntityId', 'employeeEmail', 'title', 'note',
    'startDate', 'endDate', 'hoursPerWeek', 'approverRole',
  ], 'payload');
  const requestingEntityId = token(
    payload.requestingEntityId,
    'requesting_entity',
    /^[a-z0-9][a-z0-9-]{1,47}$/,
  );
  const startDate = day(payload.startDate, false);
  const endDate = day(payload.endDate, false);
  if (endDate < startDate) {
    fail('endDate must be on or after startDate.', 400, 'ceo_command_assignment_dates_invalid');
  }
  return {
    requestingEntityId,
    employeeEmail: email(payload.employeeEmail, 'employee_email'),
    title: text(payload.title, 'title', 3, 160),
    note: text(payload.note, 'note', 1, 1_000, { multiline: true }),
    startDate,
    endDate,
    hoursPerWeek: finite(payload.hoursPerWeek, 'hours_per_week', 1, 60),
    approverRole: enumValue(payload.approverRole, new Set(['DIRECTOR', 'HR']), 'approver_role', 'DIRECTOR'),
  };
}

function normalizeIncidentAcknowledge(payload) {
  exactObject(payload, ['incidentId', 'expectedUpdatedAt'], 'payload');
  return {
    incidentId: token(payload.incidentId, 'incident_id'),
    expectedUpdatedAt: timestamp(payload.expectedUpdatedAt, 'expected_updated_at'),
  };
}
...
export function normalizeCeoCommandPayload(action, payload) {
  ...
  if (action === 'task.adjust') return normalizeTaskAdjust(payload);
  if (action === 'status.request') return normalizeStatusRequest(payload);
  if (action === 'project.status.request') return normalizeProjectStatusRequest(payload);
  ...
  if (action === 'group_workforce.request') return normalizeGroupWorkforceRequest(payload);
  if (action === 'incident.acknowledge') return normalizeIncidentAcknowledge(payload);
...
export function ceoCommandRecordHref(resource, recordId) {
  ...
  if (resource === 'tickets' && recordId) return `/tickets?focus=${encodeURIComponent(recordId)}&from=ceo-command`;
