...
import { normalizeAssignStrategy, pickOwner } from './lead-intake.js';
import { clientBusinessData, leadBusinessData, validateCrmBusinessData } from './lead-conversion-policy.js';
import {
  createReviewData, updateReviewData, createLeaveData, updateLeaveData, scalarRecordError,
  createInvoiceData, updateInvoiceData, invoiceHasPayments, validateInvoiceWrite,
  createVendorBillData, updateVendorBillData, validateVendorBillWrite,
  createTransactionData, updateTransactionData, canChangeCashEntry,
  invoiceWriteWhere, vendorBillWriteWhere, reviewWriteWhere, leaveWriteWhere,
} from './record-write-policy.js';
...
const FILTERABLE = {
  ...
  quotes: ['clientId', 'projectId', 'status'],
  activities: ['refType', 'refId'],
  tickets: ['clientId', 'status'],
  nps: ['clientId'],
  csat: ['clientId'],
...
export const RESOURCES = {
  clients: {
    ...
    orderBy: { name: 'asc' },
    beforeCreate: clientBusinessData,
    filterUpdate: clientBusinessData,
    validate: validateCrmBusinessData,
  ...
  leads: {
    ...
    filterUpdate: leadBusinessData,
    validate: validateCrmBusinessData,
    canDeleteRow: row => !row.clientId,
    canWriteRow: (row, user) => isDirector(user) || !row.ownerId || row.ownerId === user.id,
    writeWhere: row => ({ id: row.id, clientId: row.clientId ?? null, ownerId: row.ownerId, stage: row.stage }),
    // AM chỉ thấy lead mình phụ trách hoặc chưa gán; GĐ thấy tất cả
    scope: user => isDirector(user) ? {} : { OR: [{ ownerId: user.id }, { ownerId: null }] },
    // v3.4: tự chia lead chưa gán (bật trong Cài đặt).
    // v3.42: dùng chung bộ chia với cổng nhận lead tự động — nhập tay hay lead chảy về từ
    // Facebook/TikTok đều theo cùng một quy tắc, tránh hai luồng chia khác nhau.
    beforeCreate: async (data, user, prisma) => {
      data = leadBusinessData(data);
      if (data.ownerId || !prisma) return data;
      const row = await prisma.setting.findUnique({ where: { id: 1 } });
      let config = {};
      try { config = row ? JSON.parse(row.json) : {}; } catch { config = {}; }
      if (!config.autoAssignLeads) return data;
      const users = await prisma.user.findMany({ where: { status: 'active' } });
      const ams = users.filter(u => rolesOf(u).includes('AM'));
      if (!ams.length) return data;
      const routing = config.leadRouting || {};
      const candidates = await Promise.all(ams.map(async (am) => {
        const [openLeads, latest] = await Promise.all([
          prisma.lead.count({ where: { ownerId: am.id, stage: { notIn: ['won', 'lost'] } } }),
          prisma.lead.findFirst({ where: { ownerId: am.id }, orderBy: { intakeAt: 'desc' }, select: { intakeAt: true } }),
        ]);
        return { id: am.id, openLeads, lastAssignedAt: latest?.intakeAt?.toISOString() || null, ...(routing[am.id] || {}) };
      }));
      const ownerId = pickOwner(data, candidates, { strategy: normalizeAssignStrategy(config.leadAssignStrategy) });
      return ownerId ? { ...data, ownerId } : data;
    },
  },
  services: {
  ...
  reviews: {
    ...
    beforeCreate: createReviewData,
    validate: (_row, data) => scalarRecordError(data),
    ...
    filterUpdate: updateReviewData,
    writeWhere: reviewWriteWhere,
  },
  ...
  invoices: {
    ...
    orderBy: { date: 'desc' },
    beforeCreate: createInvoiceData,
    filterUpdate: updateInvoiceData,
    validate: validateInvoiceWrite,
    writeWhere: invoiceWriteWhere,
    canDeleteRow: row => !invoiceHasPayments(row),
  },
  transactions: {
    model: 'transaction', read: ['ACCOUNTANT'], write: ['ACCOUNTANT'], del: [],
    orderBy: { date: 'desc' },
    beforeCreate: createTransactionData,
    filterUpdate: updateTransactionData,
    canWriteRow: canChangeCashEntry,
    canDeleteRow: canChangeCashEntry,
  },
  vendors: {
  ...
  vendorbills: {
    ...
    orderBy: { date: 'desc' },
    beforeCreate: createVendorBillData,
    filterUpdate: updateVendorBillData,
    validate: validateVendorBillWrite,
    writeWhere: vendorBillWriteWhere,
    canDeleteRow: async (row, _user, db) => row.status === 'pending' && !row.paidDate
      && !(await db.approval.findFirst({ where: { type: 'vendorbill', refId: row.id, status: 'pending' } })),
  },
  contracts: {
  ...
  leaves: {
    ...
    canWriteRow: (row, user) => hasAny(user, ['HR']) || (row.userId === user.id && row.status === 'pending'),
    beforeCreate: createLeaveData,
    // Trạng thái chỉ đổi qua máy phê duyệt (hoặc HR/GĐ)
    filterUpdate: updateLeaveData,
    writeWhere: leaveWriteWhere,
  },
  teams: {
  ...
  users: {
    ...
    sanitize: (row, user) => {
      ...
      // v3.38: bóc luôn avatar bytes — ảnh lấy qua /api/avatar/<id>, không nhét base64 vào JSON
      const { passwordHash, totpSecret, loginFails, lockedUntil, avatar, avatarMime, ...safe } = row;
      safe.hasAvatar = !!avatar; // UI biết có ảnh để render <img> hay chữ cái đầu
  ...
  attendance: {
    ...
    // v3.41: BẰNG CHỨNG NGỮ CẢNH chỉ do /api/attendance/punch ghi. Nếu để CRUD chung sửa
    // được checkIn/checkOut/place thì mọi ràng buộc "bấm ở đâu" thành vô nghĩa — ai cũng
    // PUT thẳng giờ đẹp vào. HR vẫn sửa được ghi chú/OT/trạng thái, chỉ không sửa dấu vết.
    filterUpdate: (data, user) => {
      const { checkInIp, checkOutIp, checkInPlace, checkOutPlace, contextNote, ...safe } = data;
      if (hasAny(user, ['HR'])) return safe;
      const { userId, checkIn, checkOut, ...rest } = safe;
      return rest;
    },
  },
  candidates: {
...
// Explicit accounting locks apply to every role. Other resources retain the
// existing Director override and ownership rule on both session and API routes.
export async function canDeleteRecord(res, row, user, db) {
  const c = RESOURCES[res];
  return !!c && (c.canDeleteRow ? await c.canDeleteRow(row, user, db) : (!c.canWriteRow || isDirector(user) || c.canWriteRow(row, user)));
}
