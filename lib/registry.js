...
import { managerTargetInScope, resolveManagerScope } from './manager-scope.js';
import { scheduleConflicts, validateLiveSchedule, LIVE_TIMEZONE } from './livestream.js';
...
const FILTERABLE = {
  ...
  livesessions: ['status', 'scheduleStatus', 'date', 'hostId', 'platform', 'studio'],
};
...
export const RESOURCES = {
  ...
  tasks: {
    ...
    canWriteRowAsync: async (row, user, prisma) => {
      if (hasAny(user, ['PM']) || row.assigneeId === user.id) return true;
      if (!rolesOf(user).includes('LEAD') || !row.assigneeId || !prisma) return false;
      const [scope, target] = await Promise.all([
        resolveManagerScope(prisma, user),
        prisma.user.findUnique({ where: { id: row.assigneeId }, select: { id: true, teamId: true } }),
      ]);
      return managerTargetInScope(scope, target, user);
    },
    beforeCreate: async (data, user, prisma) => {
      const safe = withoutTaskExecutionMetadata(data);
      if (hasAny(user, ['PM'])) return safe;
      if (rolesOf(user).includes('LEAD')) {
        if (!safe.assigneeId) return safe;
        const [scope, target] = await Promise.all([
          resolveManagerScope(prisma, user),
          prisma.user.findUnique({ where: { id: safe.assigneeId }, select: { id: true, teamId: true } }),
        ]);
        if (!managerTargetInScope(scope, target, user)) throw new Error('Trưởng nhóm chỉ được giao việc trong team mình quản lý.');
        return safe;
      }
      return { ...safe, assigneeId: user.id };
    },
    filterUpdate: (data, user) => {
      const safe = withoutTaskExecutionMetadata(data);
      if (!hasAny(user, ['PM'])) { const { assigneeId, ...rest } = safe; return rest; }
      return safe;
    },
  ...
  livesessions: {
    ...
    orderBy: { date: 'desc' },
    beforeCreate: data => ({ ...data, department: 'egolive', timezone: data.timezone || LIVE_TIMEZONE }),
    filterUpdate: data => { const { department, ...safe } = data; return safe; },
    validate: async (row, data, prisma) => {
      const candidate = { ...(row || {}), ...data };
      const errors = validateLiveSchedule(candidate);
      if (errors.length) {
        const messages = {
          title_required: 'Ca đã xác nhận phải có tên chương trình.',
          start_required: 'Ca đã xác nhận phải có giờ bắt đầu.',
          start_invalid: 'Giờ bắt đầu không hợp lệ (định dạng HH:MM).',
          duration_required: 'Ca đã xác nhận phải có thời lượng.',
          duration_invalid: 'Thời lượng ca phải từ 30 phút đến 13 giờ.',
          host_required: 'Ca đã xác nhận phải có host chính.',
          studio_required: 'Ca đã xác nhận phải có studio.',
          products_required: 'TikTok LIVE đã xác nhận phải có ít nhất một sản phẩm.',
        };
        return messages[errors[0].code] || 'Lịch live chưa hợp lệ.';
      }
      if (!prisma || candidate.scheduleStatus !== 'confirmed' || !candidate.date) return null;
      const existing = await prisma.liveSession.findMany({
        where: { date: candidate.date, status: { not: 'cancelled' } },
        select: {
          id: true, date: true, startAt: true, durationMin: true, status: true,
          hostId: true, assistantId: true, operatorId: true, moderatorId: true, studio: true,
        },
      });
      const conflicts = scheduleConflicts(candidate, existing);
      if (!conflicts.length) return null;
      const resources = [...new Set(conflicts.map((item) => item.kind === 'studio' ? `studio ${item.studio}` : 'nhân sự trong ekip'))];
      return `Trùng lịch ${resources.join(' và ')} với một ca live khác.`;
    },
  },
  violations: {
