...
function safeApproval(approval, proposal = null) {
  return {
    id: approval.id,
    type: approval.type,
...
export async function listRealmLaunchApprovals(db, sessionUser, { secret } = {}) {
  requireDirector(sessionUser);
  ...
  const rows = approvals.map((approval) => {
    try { return safeApproval(approval, openProposal(approval.payload, secret)); }
    catch { return safeApproval(approval); }
  });
  return {
