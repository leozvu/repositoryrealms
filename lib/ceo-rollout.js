...
export const CEO_ROLLOUT_RING_DEFINITIONS = Object.freeze({
  ...
  read_only: Object.freeze({ capabilities: ['dashboard.read', 'deep_links.read', 'decisions.read'], label: 'Read-only pilot' }),
  ceo_sso: Object.freeze({ capabilities: ['dashboard.read', 'deep_links.read', 'decisions.read', 'sso.issue', 'federation.read'], label: 'CEO-only SSO' }),
  messaging: Object.freeze({ capabilities: ['dashboard.read', 'deep_links.read', 'decisions.read', 'sso.issue', 'federation.read', 'messaging.sync', 'messaging.send'], label: 'Cross-entity messaging' }),
  commands: Object.freeze({ capabilities: ['dashboard.read', 'deep_links.read', 'decisions.read', 'sso.issue', 'federation.read', 'messaging.sync', 'messaging.send', 'command.dispatch'], label: 'Allowlisted commands' }),
});
...
export function requiredCeoRolloutEvidence(entityId, ring) {
  ...
  if (['egoric', 'egolive'].includes(entityId) && ring === 'commands') required.push('finance_review');
  return required;
}
...
export function evaluateCeoRolloutEvidence(entityId, ring, rows = [], { now = new Date(), transitionActorId = null } = {}) {
  ...
  const independentKinds = ring === 'commands'
    ? ['maker_checker', ...(['egoric', 'egolive'].includes(entityId) ? ['finance_review'] : [])]
    : [];
...
export function readCeoRolloutApproval(env = process.env, now = new Date()) {
  ...
  // v3.38: nhận danh sách canary phân tách bằng dấu phẩy — sau giai đoạn bake với 1 công ty
  // đầu tiên, CEO có thể mở ring commands cho thêm công ty mà không phải sửa code.
  // Giá trị đơn (1 entity) vẫn hoạt động y hệt như trước.
  const commandCanaryEntities = String(env.CEO_ROLLOUT_COMMAND_CANARY_ENTITY || '')
    .split(',').map((item) => item.trim().toLowerCase()).filter(Boolean);
  const commandCanaryEntity = commandCanaryEntities[0] || null;
  return {
    decision,
...
export function assertCeoProductionRolloutApproval(entity, targetRing, approval, now = new Date()) {
  ...
  const canaries = approval.commandCanaryEntities || (approval.commandCanaryEntity ? [approval.commandCanaryEntity] : []);
  if (targetRing === 'commands' && !canaries.includes(entity.id)) {
...
export async function assertCeoRolloutCapability(db, entityId, capability, { action = null, now = new Date() } = {}) {
  ...
  if (['egoric', 'egolive'].includes(entityId) && action && /(?:payout|settlement)/i.test(action)) {
    fail('Egolive department payout and settlement remain local to Egoric.', 403, 'ceo_rollout_egolive_finance_local_only');
  }
