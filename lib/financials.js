...
export function incomeStatement(txs, match) {
  ...
  const finIncome = inc.filter(t => FIN_INCOME_CATS.includes(t.category)).reduce((s, t) => s + transactionAmountVnd(t), 0);
  const opRevenue = inc.reduce((s, t) => s + transactionAmountVnd(t), 0) - finIncome;
  const finExpense = exp.filter(t => FIN_EXPENSE_CATS.includes(t.category)).reduce((s, t) => s + transactionAmountVnd(t), 0);
  const opexByCat = {};
  exp.filter(t => !FIN_EXPENSE_CATS.includes(t.category)).forEach(t => { const c = t.category || 'Khác'; opexByCat[c] = (opexByCat[c] || 0) + transactionAmountVnd(t); });
...
export function cashFlow(txs, match) {
  ...
  txs.filter(t => match(t.date)).forEach(t => {
    ...
    const c = t.category || 'Khác'; map[c] = (map[c] || 0) + transactionAmountVnd(t);
  });
...
export function cashBalanceAsOf(txs, asOfDate) {
  return txs.filter(t => t.date && t.date <= asOfDate)
    .reduce((s, t) => s + (t.type === 'income' ? 1 : -1) * transactionAmountVnd(t), 0);
}
