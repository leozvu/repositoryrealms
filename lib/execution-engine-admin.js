...
import { managerTargetInScope, managerTaskInScope, resolveManagerScope } from './manager-scope.js';
...
function managerTaskScope(task, user, scope = null) {
  if (scope) return managerTaskInScope(scope, task, user);
  if (task?.assigneeId === user.id) return true; // việc của chính mình luôn trong phạm vi
  const guildScope = realmGuildScope(user);
  if (guildScope.kind === 'company') return true;
  return guildScope.kind === 'team' && task?.assignee?.teamId === guildScope.teamId;
}

function managerOwnerScope(owner, user, scope = null) {
  if (scope) return managerTargetInScope(scope, owner, user);
  if (owner?.id === user.id) return true; // hàng đợi của chính mình
  const guildScope = realmGuildScope(user);
  if (guildScope.kind === 'company') return true;
  return guildScope.kind === 'team' && owner?.teamId === guildScope.teamId;
}
...
async function scopedTask(db, user, id, managerScope = null) {
  const task = await db.task.findUnique({ where: { id }, include: { assignee: { select: { id: true, teamId: true } } } });
  if (!task || !managerTaskScope(task, user, managerScope)) fail('Không tìm thấy Task trong phạm vi điều phối của bạn.', 404, 'execution_task_not_found');
  return task;
}
...
async function reprioritize(db, user, command, managerScope = null) {
  const owner = await db.user.findUnique({ where: { id: command.ownerId }, select: { id: true, teamId: true, status: true, userType: true } });
  if (!owner || owner.status !== 'active' || owner.userType !== 'employee' || !managerOwnerScope(owner, user, managerScope)) fail('Người sở hữu hàng đợi không thuộc phạm vi điều phối.', 403, 'execution_queue_owner_forbidden');
  const before = await scopedTask(db, user, command.entityId, managerScope);
...
async function mutateSingleTask(db, user, command, now, managerScope = null) {
  const before = await scopedTask(db, user, command.entityId, managerScope);
  assertMutable(before);
...
async function splitTask(db, user, command, now, managerScope = null) {
  const before = await scopedTask(db, user, command.entityId, managerScope);
  assertMutable(before);
  assertVersion(before, command.expectedVersion);
  const result = await db.$transaction(async (tx) => {
...
async function mergeTasks(db, user, command, now, managerScope = null) {
  const sources = await db.task.findMany({ where: { id: { in: command.sourceTaskIds } }, include: { assignee: { select: { id: true, teamId: true } } } });
  if (sources.length !== command.sourceTaskIds.length || sources.some((task) => !managerTaskScope(task, user, managerScope))) fail('Không tìm thấy đủ Task nguồn trong phạm vi điều phối.', 404, 'execution_merge_sources_not_found');
...
export async function executeExecutionAction(db, user, input, now = new Date()) {
  ...
  const selfQueueAction = input?.action === 'task.reprioritize' && input?.ownerId && input.ownerId === user?.id;
  if (selfQueueAction) requireSelfQueueActor(user);
  else requireManager(user);
  const managerScope = selfQueueAction ? null : await resolveManagerScope(db, user);
  if (!selfQueueAction && managerScope.kind === 'none') fail('Trưởng nhóm cần được gắn Team hoặc đứng tên trưởng Team trước khi điều phối.', 403, 'execution_manager_scope_missing');
  const command = normalizeExecutionCommand(input);
  const prior = await existingReceipt(db, user, command);
  if (prior) return prior;
  try {
    if (command.action === 'task.reprioritize') return await reprioritize(db, user, command, managerScope);
    if (command.action === 'task.split') return await splitTask(db, user, command, now, managerScope);
    if (command.action === 'task.merge') return await mergeTasks(db, user, command, now, managerScope);
    return await mutateSingleTask(db, user, command, now, managerScope);
  } catch (error) {
...
export async function loadTeamWork(db, user, now = new Date()) {
  requireManager(user);
  const scope = await resolveManagerScope(db, user);
  if (scope.kind === 'none') fail('Trưởng nhóm cần được gắn Team hoặc đứng tên trưởng Team trước khi điều phối.', 403, 'execution_manager_scope_missing');
  const memberWhere = scope.kind === 'company'
    ? { status: 'active', userType: 'employee' }
    : { teamId: { in: scope.teamIds }, status: 'active', userType: 'employee' };
  ...
  return {
    ...buildTeamWorkReadModel({ members, tasks: resource.tasks, queueStates, today: now.toISOString().slice(0, 10), scope: scope.kind === 'company' ? 'company' : 'team' }),
    resourceIntelligence: resource.summary,
    generatedAt: now.toISOString(),
    source: 'erp-task',
  };
