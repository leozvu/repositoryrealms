...
export async function loadCeoCommandCapabilities(db, user, rawToken, entityId, context = {}) {
  const now = context.now || new Date();
  const session = await requireCommandSession(db, user, rawToken, { ...context, now, touch: false });
  const entity = await db.ceoEntityRegistry.findUnique({ where: { id: String(entityId || '').trim().toLowerCase() } });
  if (!entity || !entity.enabled) throw new CeoCommandError('Target entity is not enabled.', 403, 'ceo_command_entity_disabled');
  const membership = await db.ceoEntityMembership.findUnique({
    where: { identityId_entityId: { identityId: session.identityId, entityId: entity.id } },
  });
  if (!membership || membership.status !== 'active' || membership.localRole !== 'DIRECTOR') {
    throw new CeoCommandError('Active Director membership is required.', 403, 'ceo_command_membership_required');
  }
  ...
  try {
    const response = await fetchImpl(new URL('/api/ceo/v1/capabilities', upstreamOrigin), {
      method: 'GET',
      headers: {
    ...
    if (!response.ok) throw new CeoCommandError('Entity capabilities are unavailable.', 502, `ceo_command_capabilities_http_${response.status}`);
    const body = await readBoundedJson(response);
    if (body?.entityId !== entity.id || !Array.isArray(body?.capabilities?.commands)) {
      throw new CeoCommandError('Entity returned an invalid capability contract.', 502, 'ceo_command_capabilities_invalid');
    }
    const allowlisted = new Set(CEO_COMMAND_ACTIONS);
    const actions = [...new Set(body.capabilities.commands
      .map((item) => String(item?.action || '').trim())
      .filter((action) => allowlisted.has(action)))];
    ...
    return {
      ...
      actions,
      source: 'entity',
      asOf: body.asOf || now.toISOString(),
    };
  } catch (error) {
    const code = safeErrorCode(error, 'ceo_command_capabilities_unavailable');
    await recordCeoRegistrySyncFailure(db, entity.id, code, now).catch(() => {});
    if (error instanceof CeoCommandError) throw error;
    throw new CeoCommandError('Entity capabilities are unavailable.', 503, code);
  } finally {
    clearTimeout(timer);
  }
}
