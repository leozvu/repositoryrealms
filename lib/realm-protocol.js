...
export function createRealmDemoGuestProfile(value = {}, seed = '') {
  const profile = normalizeProfile(value);
  if (profile.name !== DEFAULT_PROFILE.name) return profile;
  const suffix = String(seed || '')
    ...
    .slice(-4)
    .padStart(4, '0');
  return { ...profile, name: `Adventurer ${suffix}` };
}
...
export function mergeRealmPresencePeople({ staff = [], remotePlayers = [], selfProfile = null } = {}) {
  ...
  const selfName = typeof selfProfile?.name === 'string' && selfProfile.name.trim()
    ...
    : '';
  const seen = new Set([selfIdentity].filter(Boolean));
  const names = new Map(selfName ? [[selfName, new Set([selfIdentity || selfName])]] : []);
  ...
  for (const person of [...remoteFirst, ...(Array.isArray(staff) ? staff : [])]) {
    ...
    const knownIdentity = identity.startsWith('user:');
    const sameName = names.get(nameIdentity);
    // Equal display names never merge two authenticated people. Name fallback
    // remains for legacy directory/demo entries that do not carry a stable user ID.
    if (seen.has(identity) || (sameName && (!knownIdentity || sameName.has(nameIdentity)))) continue;
    seen.add(identity);
    if (nameIdentity) { const owners = names.get(nameIdentity) || new Set(); owners.add(identity); names.set(nameIdentity, owners); }
    people.push(person);
  }
  return people;
}
