...
export function buildRealmPilotOperationAlerts({ policy, operations, readiness, rehearsal, now = new Date() } = {}) {
...
export async function loadRealmPilotOperationsDashboard(db, sessionUser, { now = new Date() } = {}) {
  ...
  return {
    ...
    rehearsal,
    alerts: buildRealmPilotOperationAlerts({ policy, operations, readiness, rehearsal, now }),
