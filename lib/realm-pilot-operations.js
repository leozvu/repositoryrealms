...
export function buildRealmPilotOperationAlerts({ policy, operations, readiness, now = new Date() } = {}) {
...
export async function loadRealmPilotOperationsDashboard(db, sessionUser, { now = new Date() } = {}) {
  ...
  return {
    ...
    alerts: buildRealmPilotOperationAlerts({ policy, operations, readiness, now }),
...
async function persistOperations(tx, setting, operations, user, action, wave, detail) {
  ...
  await tx.auditLog.create({
    data: {
      ...
      refId: wave.id,
      detail: `${detail}; wave ${wave.id}; status ${wave.status}; policy v${wave.policyVersion}; eligible ${wave.eligibleUsers}; fallback ${wave.fallbackUsers}; no roster`,
    },
  });
  return next;
}
...
export async function transitionRealmPilotWave(db, sessionUser, input = {}, { now = new Date() } = {}) {
  ...
  if (!['submit', 'approve', 'reject', 'pause', 'complete'].includes(action)) {
  ...
  return db.$transaction(async (tx) => {
    ...
    if (action === 'submit') {
      ...
      wave = { ...wave, status: 'awaiting_approval', ...waveSnapshot(policy, readiness.metrics), submittedAt: now.toISOString(), submittedById: sessionUser.id, submittedByName: sessionUser.name, decisionNote: null };
    ...
    if (action === 'approve') {
      ...
      wave = { ...wave, status: 'active', ...waveSnapshot(policy, readiness.metrics), activatedAt: now.toISOString(), approvedById: sessionUser.id, approvedByName: sessionUser.name, decisionNote: cleanText(input.note, 240) || null };
    ...
    if (action === 'pause' || action === 'complete') {
      ...
      if (action === 'pause') {
        wave = { ...wave, status: 'paused', pausedAt: now.toISOString(), decisionNote: cleanText(input.note, 240) || 'Wave được tạm dừng; cohort quay về ERP.' };
