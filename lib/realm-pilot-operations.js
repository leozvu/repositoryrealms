  ...
  return db.$transaction(async (tx) => {
    ...
    if (action === 'report_incident') {
      ...
      if (severity === 'critical') {
        ...
        notificationCount += await createNotifications(tx, memberIds, `Pilot wave “${wave.name}” đã rollback về ERP do incident vận hành. Tiếp tục công việc trên ERP · CRM; dữ liệu được giữ nguyên.`, '/dashboard');
      }
      notificationCount += await createNotifications(tx, await directorIds(tx, sessionUser.id), `${severity === 'critical' ? 'Critical' : 'Warning'} incident trong pilot wave “${wave.name}”: ${category.label}.`, '/settings#realm-pilot-operations-title');
    ...
    if (action === 'monitor_incident' || action === 'resolve_incident') {
      ...
      notificationCount = await createNotifications(tx, await directorIds(tx, sessionUser.id), `Incident “${INCIDENT_CATEGORY_MAP.get(incident.category).label}” đã chuyển sang ${incident.status === 'resolved' ? 'đã khống chế' : 'đang theo dõi'}.`, '/settings#realm-pilot-operations-title');
    }
    ...
    const next = await persistOperations(tx, setting, operations, sessionUser, action, wave, `${action} pilot wave; incident ${incident?.category || 'none'}; notifications ${notificationCount}`);
    return { operations: next, wave, incident, policy, notificationCount };
  }, { isolationLevel: 'Serializable' });
}
