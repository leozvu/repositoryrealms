...
async function readPresence(db, session, entity, context) {
  ...
  // Đợt 1 (24/7): 3 lệnh chuẩn bị dưới đây từng nằm NGOÀI try — một entity bị circuit-open
  // hoặc origin không tin cậy sẽ ném ra ngoài Promise.all và làm sập TOÀN BỘ bản đồ
  // (world?entity=all → 503) dù các công ty khác vẫn khỏe. Nay lỗi chuẩn bị được quy về
  // trạng thái degraded của riêng kingdom đó.
  let origin;
  let prepared;
  try {
    await assertCeoRolloutCapability(db, entity.id, 'federation.read', { now });
    origin = assertCeoDashboardUpstreamOrigin(entity, context.allowedOriginResolver(entity));
    prepared = await prepareCeoRegistrySync(db, entity.id, { now, secretResolver: context.secretResolver });
  } catch (error) {
...
export async function loadCeoFederationWorld(db, user, rawToken, { entityId = 'all' } = {}, context = {}) {
  ...
  // The world map is a product surface for the four operating companies, not a
  // raw registry browser. Staging/sandbox memberships remain visible in the CEO
  // registry, but must never become an extra "kingdom" on this map.
  const eligible = memberships.filter((membership) => (
    membership.localRole === 'DIRECTOR'
    && membership.entity?.environment === 'production'
    && Object.hasOwn(CEO_FEDERATION_KINGDOMS, membership.entityId)
    && normalizeCeoScopes(membership.scopes).includes(CEO_FEDERATION_SCOPE)
  ));
