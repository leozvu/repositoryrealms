...
async function ensureIdentity(tx, user, now) {
  ...
  const entityIds = new Set(existing.map((membership) => membership.entityId));
  for (const entity of entities) {
    ...
    await tx.ceoEntityMembership.create({
      data: {
        ...
        localRole: 'DIRECTOR',
        scopes: JSON.stringify(capabilitiesToCeoScopes(parseCeoRegistryCapabilities(entity.capabilities))),
        status: 'active',
      },
    });
  }
...
export async function recoverCeoPortalAccount(db, input = {}, context = {}) {
  ...
  const identity = await db.ceoGlobalIdentity.findUnique({ where: { userId: user.id } });
  if (!identity || identity.status !== 'active') { await recordRecoveryFailure(db, user, now); throw generic(); }
  let codeHash;
  ...
  const created = await db.$transaction(async (tx) => {
    ...
    const next = await createSession(tx, identity, { ...context, now, deviceLabel: input.deviceLabel, assuranceLevel: 'recovery', stepUp: false });
    await audit(tx, user, 'ceo_recovery_used', next.session.id, `recoveryVersion=${identity.recoveryVersion}`);
    return next;
  }, { isolationLevel: 'Serializable' });
  return { user: { id: user.id, email: user.email, name: user.name, role: user.role, roles: rolesOf(user), teamId: user.teamId, userType: user.userType }, token: created.rawToken, identity, session: created.session };
}
