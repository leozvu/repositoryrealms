...
import { buildProjectExecutionHealth } from './project-execution-health.js';
...
export function projectStats({
  ...
  const execution = buildProjectExecutionHealth({
    project,
    tasks: pt,
    companyTasks: tasks,
    timeLogs: pl,
    ...
    phases: phases.filter((row) => row.projectId === project.id),
    vendorBills: (vendorBills || []).filter((row) => row.projectId === project.id),
  ...
  const out = {
    ...
    taskOverdue: pt.filter(t => t.status !== 'done' && t.dueDate && t.dueDate < effectiveToday).length,
    health: execution.health.level,
    healthReasons: execution.health.signals.map((signal) => signal.label),
    deliveryRisk: {
      level: execution.health.level,
      label: execution.health.label,
      confidence: execution.health.confidence.band,
      primarySignal: execution.health.signals[0]?.label || null,
    },
    blockedTasks: execution.delivery.blocked,
    waitingTasks: execution.delivery.waiting,
    dependencyBlocked: execution.delivery.unresolvedDependencies,
    unassignedTasks: execution.delivery.unassigned,
    constrainedMembers: execution.capacity.constrainedMembers,
    estimateCoverage: execution.resource.estimateCoveragePercent,
    source: 'canonical-erp-project',
  };
  if (canSeeMoney) {
    out.cost = execution.financial.planningCostProxy;
    out.labor = execution.financial.laborAccrued;
    out.vendor = execution.financial.vendorCommitted;
    out.budget = project.budget;
    out.margin = execution.financial.planningMarginProxy;
    out.marginBasis = execution.financial.costBasis;
    out.isAccountingProfit = false;
  }
  return out;
}
