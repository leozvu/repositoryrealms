...
export async function reconcileSchemaContract({ db, schema, expectedBeforeSha256 }) {
  return db.$transaction(async (tx) => {
    ...
    const [lock] = await tx.$queryRawUnsafe("SELECT pg_try_advisory_xact_lock(hashtext('repositoryrealms:ceo-schema-reconcile')) AS acquired");
    if (!lock?.acquired) throw new CeoSchemaReconcileError('Another schema reconciliation is already running.', 409, 'ceo_schema_reconcile_busy');
