...
async function fetchOne(db, entity, context) {
  ...
  let timer;
  try {
    // Registry/circuit/rollout failures belong to this entity. Keep them inside
    // the isolation boundary so one unhealthy company cannot take down the
    // entire CEO workspace.
    ...
    if (!['ceo_registry_entity_disabled', 'ceo_registry_circuit_open'].includes(errorCode)) {
