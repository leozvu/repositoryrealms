...
export function createRealmEmbassyDashboard({
  ...
  const activityEvidence = Array.isArray(workloadActivities) ? workloadActivities : leads.flatMap((lead) => (
    Array.isArray(lead?.activities) ? lead.activities.map((activity) => ({ ...activity, refId: lead.id })) : []
  ));
  const workloadIntelligence = buildCrmWorkloadIntelligence({
    leads,
