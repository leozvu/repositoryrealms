...
async function targetRequest(db, entity, path, init, context = {}) {
  ...
  try {
    const response = await (context.fetchImpl || fetch)(new URL(path, origin), {
      ...init, headers: { Accept: 'application/json', Authorization: `Bearer ${prepared.credential}`, 'User-Agent': 'RepositoryRealms-CEO-Portal/1.0', 'X-CEO-Entity-ID': entity.id, ...(init?.headers || {}) },
...
async function syncEntityDirectory(db, user, session, entity, context) {
  const now = context.now || new Date();
  try { await assertCeoRolloutCapability(db, entity.id, 'messaging.sync', { now }); }
  catch (error) { throw new CeoMessagingError(error.message, error.status || 423, error.code || 'ceo_rollout_capability_hold'); }
...
export async function sendCeoMessage(db, user, rawToken, conversationId, input = {}, context = {}) {
  ...
  try { await assertCeoRolloutCapability(db, entity.id, 'messaging.send', { now }); }
  catch (error) { throw new CeoMessagingError(error.message, error.status || 423, error.code || 'ceo_rollout_capability_hold'); }
  ...
  await db.$transaction(async (tx) => {
    for (const item of feed.messages) {
      ...
      await tx.notification.create({ data: {
        userId: user.id,
        text: `Phản hồi từ ${item.senderName} · ${entity.displayName}`,
        route: `/ceo-inbox?conversation=${encodeURIComponent(conversation.id)}`,
        createdAt: now,
      } });
      imported += 1;
    }
...
export async function syncCeoInbox(db, user, rawToken, { limit = 12 } = {}, context = {}) {
  const now = context.now || new Date();
  const session = await requireSession(db, user, rawToken, { ...context, scope: 'message.read', now });
  const take = Math.max(1, Math.min(Number(limit) || 12, 20));
  const conversations = await db.ceoUnifiedConversation.findMany({
    where: { identityId: session.identityId, status: 'active' },
    select: { id: true },
    orderBy: [{ lastMessageAt: 'desc' }, { updatedAt: 'desc' }],
    take,
  });
  const summary = { checked: 0, imported: 0, readReceipts: 0, degraded: 0 };
  for (const conversation of conversations) {
    try {
      const result = await refreshCeoConversation(db, user, rawToken, conversation.id, { ...context, now, touch: false });
      summary.checked += 1;
      summary.imported += Number(result.imported || 0);
      summary.readReceipts += Number(result.readReceipts || 0);
    } catch {
      summary.checked += 1;
      summary.degraded += 1;
    }
  ...
  return {
    version: CEO_MESSAGING_VERSION,
    asOf: now.toISOString(),
    ...summary,
    partial: summary.degraded > 0,
  };
}
