...
export async function saveRealmPilotConfig(db, sessionUser, value, {
  ...
  return db.$transaction(async (tx) => {
    ...
    if (requireLaunchPreview && draft.mode !== 'off' && typeof verifyLaunchPreview !== 'function') {
      throw new RealmOperationError('Hãy chạy dry-run trước khi áp dụng chính sách.', 428, 'realm_launch_preview_required');
    }
    const launchPreview = requireLaunchPreview && draft.mode !== 'off'
