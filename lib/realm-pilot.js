...
export function normalizeRealmPilotConfig(value = {}) {
  ...
  return { mode, defaultSurface, roles, features, onboardingVersion, version };
}
  ...
  return db.$transaction(async (tx) => {
    ...
    await tx.auditLog.create({
      data: {
        ...
        action: 'update',
        entity: 'realm_pilot',
        detail: `v${config.version}; mode ${config.mode}; default ${config.defaultSurface}; roles ${config.roles.join(',') || 'none'}; features ${REALM_PILOT_FEATURES.filter((feature) => config.features[feature]).join(',') || 'none'}`,
      },
    });
    return config;
