...
async function adjustTask(tx, command) {
  if (!canWrite('tasks', command.user)) {
    throw new CeoCommandError('Task write is not authorized.', 403, 'ceo_command_task_forbidden');
  }
  const payload = command.payload;
  const current = await tx.task.findUnique({
    ...
    select: { id: true, assigneeId: true, status: true, updatedAt: true },
  });
  if (!current) throw new CeoCommandError('Task does not exist in the target entity.', 404, 'ceo_command_task_not_found');
  if (['done', 'cancelled'].includes(current.status)) {
    throw new CeoCommandError('Completed or cancelled tasks cannot be adjusted here.', 409, 'ceo_command_task_terminal');
  }
  const assignee = Object.hasOwn(payload, 'assigneeEmail')
    ? await requireActiveUserByEmail(tx, payload.assigneeEmail)
    : null;
  const draft = {
    ...(assignee ? { assigneeId: assignee.id } : {}),
    ...(Object.hasOwn(payload, 'dueDate') ? { dueDate: payload.dueDate } : {}),
    ...(Object.hasOwn(payload, 'priority') ? { priority: payload.priority } : {}),
    ...(Object.hasOwn(payload, 'estHours') ? { estHours: payload.estHours } : {}),
  };
  const data = RESOURCES.tasks.filterUpdate ? RESOURCES.tasks.filterUpdate(draft, command.user) : draft;
  if (!Object.keys(data).length) throw new CeoCommandError('No authorized task field remains.', 403, 'ceo_command_task_adjust_forbidden');
  if (RESOURCES.tasks.validate) {
    const validationError = await RESOURCES.tasks.validate(current, data, tx);
    if (validationError) throw new CeoCommandError(validationError, 422, 'ceo_command_task_rule_failed');
  }
  const updated = await tx.task.updateMany({
    where: { id: current.id, updatedAt: new Date(payload.expectedUpdatedAt), status: { notIn: ['done', 'cancelled'] } },
    data,
  });
  if (updated.count !== 1) {
    throw new CeoCommandError('Task changed after it was reviewed. Reload before retrying.', 409, 'ceo_command_task_version_conflict');
  }
  const row = await tx.task.findUnique({ where: { id: current.id } });
  return { row, resource: 'tasks', recordId: current.id, resultCount: 1, event: 'update' };
}

async function requestProjectStatus(tx, command) {
  ...
  return createTask(tx, {
    ...
    payload: {
      ...
      targetEmail: command.payload.targetEmail,
...
async function acknowledgeIncident(tx, command) {
  if (!canWrite('tickets', command.user)) {
    throw new CeoCommandError('Incident write is not authorized.', 403, 'ceo_command_incident_forbidden');
  }
  const current = await tx.ticket.findUnique({
    where: { id: command.payload.incidentId },
    select: { id: true, status: true, source: true, updatedAt: true },
  });
  if (!current || current.source !== 'incident_registry') {
    throw new CeoCommandError('Incident is unavailable.', 404, 'ceo_command_incident_not_found');
  }
  if (current.status !== 'open') {
    throw new CeoCommandError('Only open incidents can be acknowledged.', 409, 'ceo_command_incident_state_conflict');
  }
  const updated = await tx.ticket.updateMany({
    where: { id: current.id, source: 'incident_registry', status: 'open', updatedAt: new Date(command.payload.expectedUpdatedAt) },
    data: { status: 'in_progress' },
  });
  if (updated.count !== 1) {
    throw new CeoCommandError('Incident changed after it was reviewed. Reload before retrying.', 409, 'ceo_command_incident_version_conflict');
  }
  const row = await tx.ticket.findUnique({ where: { id: current.id } });
  return { row, resource: 'tickets', recordId: current.id, resultCount: 1, event: 'update' };
}
...
async function executeBusinessMutation(tx, command, now) {
  ...
  if (command.action === 'task.adjust') return adjustTask(tx, command);
  if (command.action === 'status.request') return createTask(tx, command, { statusRequest: true });
  if (command.action === 'project.status.request') return requestProjectStatus(tx, command);
  ...
  if (command.action === 'incident.acknowledge') return acknowledgeIncident(tx, command);
