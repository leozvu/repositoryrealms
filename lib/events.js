...
export async function notify(userIds, text, route) {
  ...
  } catch { /* notification delivery must never break the business mutation */ }
}
...
async function systemAutomations(resource, event, row, old, user) {
  ...
  if (resource === 'tasks' && row?.assigneeId && row.assigneeId !== user?.id
    ...
    await notify(row.assigneeId, `Bạn được gán việc: ${row.title}`, notificationRecordRoute('tasks', row.id));
  }
  ...
  if (resource === 'tasks' && event === 'update' && row?.status === 'done' && old?.status !== 'done') {
    try {
      const result = await awardGoldForTask(row, old);
      if (result?.awarded) {
        await notify(row.assigneeId, `⭐ +${result.awarded} Gold — hoàn thành đúng hạn: ${row.title}`, '/realm');
      }
    } catch (error) { console.error('gold_award_task_failed', error?.code || error?.message); }
  }
  // Ticket khẩn được gán → báo người xử lý
  if (resource === 'tickets' && row?.assigneeId && row.assigneeId !== user?.id
    && (event === 'create' || (event === 'update' && old?.assigneeId !== row.assigneeId))) {
    await notify(row.assigneeId, `Bạn được giao ticket ${row.code}: ${row.title}`, notificationRecordRoute('tickets', row.id));
  }
  ...
  if (resource === 'taskcomments' && event === 'create' && row?.taskId) {
    ...
    if (task?.assigneeId && task.assigneeId !== user?.id) {
      await notify(task.assigneeId, `${user?.name || 'Ai đó'} bình luận việc "${task.title}": ${String(row.content).slice(0, 60)}`, notificationRecordRoute('tasks', task.id));
    }
    ...
    if (mentions) {
      const all = await prisma.user.findMany({ where: { status: 'active' }, select: { id: true, name: true } });
      const hit = new Set();
      ...
      if (targets.length) await notify(targets, `${user?.name || 'Ai đó'} nhắc bạn trong việc "${task?.title || ''}": ${String(row.content).slice(0, 60)}`, notificationRecordRoute('tasks', task?.id));
    }
  }
  ...
  if (resource === 'activities' && event === 'create' && row?.refType === 'lead' && row?.refId) {
    const lead = await prisma.lead.findUnique({ where: { id: row.refId }, select: { ownerId: true, company: true, name: true } });
    if (lead?.ownerId && lead.ownerId !== user?.id) {
      await notify(lead.ownerId, `${user?.name || 'Ai đó'} lên lịch follow-up “${String(row.title || '').slice(0, 60)}” cho ${lead.company || lead.name || 'Lead'}`, notificationRecordRoute('leads', row.refId));
    }
  }
  ...
  if (resource === 'tasks' && (event === 'create' || (event === 'update' && old && old.status !== row.status))) {
    ...
    if (row.statusSince !== today) await prisma.task.update({ where: { id: row.id }, data: { statusSince: today } }).catch(() => {});
  }
  ...
  if (resource === 'tasks' && event === 'update' && row?.recur && row.status === 'done' && old?.status !== 'done') {
    ...
    if (row.assigneeId) await notify(row.assigneeId, `Việc định kỳ "${row.title}" đã tạo kỳ tiếp theo (hạn ${nextDue})`, notificationRecordRoute('tasks', nextTask.id));
  }
...
async function step(name, fn, resource, row, user) {
  try {
    ...
    console.error(`[automation] ${name} lỗi khi ${resource}:`, e?.message || e);
    await prisma.auditLog.create({
      data: {
        userId: user?.id || 'system', userName: user?.name || 'hệ thống',
        action: 'automation_error', entity: resource, refId: row?.id || null,
        detail: `Tự động hóa "${name}" lỗi: ${String(e?.message || e).slice(0, 200)}`,
      },
