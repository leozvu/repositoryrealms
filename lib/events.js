...
async function systemAutomations(resource, event, row, old, user) {
  ...
  if (resource === 'taskcomments' && event === 'create' && row?.taskId) {
    ...
    if (mentions) {
      ...
      if (targets.length) await notify(targets, `${user?.name || 'Ai đó'} nhắc bạn trong việc "${task?.title || ''}": ${String(row.content).slice(0, 60)}`, '/tasks');
    }
  }
  ...
  if (resource === 'activities' && event === 'create' && row?.refType === 'lead' && row?.refId) {
    ...
    if (lead?.ownerId && lead.ownerId !== user?.id) {
      await notify(lead.ownerId, `${user?.name || 'Ai đó'} lên lịch follow-up “${String(row.title || '').slice(0, 60)}” cho ${lead.company || lead.name || 'Lead'}`, '/leads');
    }
  }
