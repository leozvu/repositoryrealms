...
export const CEO_ENTITY_REGISTRY_SEED = Object.freeze([
  {
    ...
    environment: 'production', credentialRef: 'CEO_ENTITY_AIM_API_KEY', serviceCredentialRef: 'CEO_ENTITY_AIM_SERVICE_KEY',
  },
  {
    id: 'egoric', displayName: 'Egoric Agency', baseUrl: 'https://erp-egoric.vercel.app',
    businessProfile: 'agency', capabilities: ['finance', 'crm', 'delivery', 'support', 'people'],
    environment: 'production', credentialRef: 'CEO_ENTITY_EGORIC_API_KEY', serviceCredentialRef: 'CEO_ENTITY_EGORIC_SERVICE_KEY',
  },
  {
    ...
    environment: 'production', credentialRef: 'CEO_ENTITY_VNECOM_API_KEY', serviceCredentialRef: 'CEO_ENTITY_VNECOM_SERVICE_KEY',
  },
  {
    ...
    environment: 'production', credentialRef: 'CEO_ENTITY_EGOLIVE_API_KEY', serviceCredentialRef: 'CEO_ENTITY_EGOLIVE_SERVICE_KEY',
  },
]);
...
export function normalizeCeoServiceCredentialRef(value) {
  const normalized = String(value || '').trim();
  if (!/^CEO_ENTITY_[A-Z0-9_]+_SERVICE_KEY$/.test(normalized) || normalized.length > 96) {
    throw new CeoRegistryError(
      'serviceCredentialRef must name a dedicated scoped CEO service key environment variable.',
      400,
      'ceo_registry_service_credential_ref_invalid',
    );
  }
  return normalized;
}
...
export function normalizeCeoServiceCredentialRotation(input = {}) {
  if (!input || typeof input !== 'object' || Array.isArray(input)) {
    throw new CeoRegistryError('Service credential rotation is invalid.', 400, 'ceo_registry_service_rotation_invalid');
  }
  return {
    expectedVersion: integer(input.expectedVersion, 'expected_version'),
    serviceCredentialRef: normalizeCeoServiceCredentialRef(input.serviceCredentialRef),
  };
}
...
export function serializeCeoRegistryEntity(row, { secretResolver = (name) => process.env[name] } = {}) {
  ...
  const serviceCredentialConfigured = Boolean(row?.serviceCredentialRef && secretResolver(row.serviceCredentialRef));
  return {
    ...
    serviceCredential: {
      configured: serviceCredentialConfigured,
      separated: Boolean(row?.serviceCredentialRef && row.serviceCredentialRef !== row.credentialRef),
      version: Number(row.serviceCredentialVersion || 1),
      rotatedAt: row.serviceRotatedAt || null,
    },
    sync: {
