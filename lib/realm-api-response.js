...
export function realmErrorResponse(trace, error, {
  ...
  const publicError = Number.isInteger(status) && status >= 400 && status < 500 && typeof error?.code === 'string';
  const responseStatus = publicError ? status : 500;
  const code = publicError ? error.code : fallbackCode;
  if (!publicError) safeRealmException(trace, error, code);
  return realmJsonResponse(trace, {
    ...
    code,
    outcome: publicError ? 'rejected' : 'error',
