...
import { applyTokenAccessToSession, syncTokenAccess } from './auth-token.js';
import { verifyTotp } from './totp.js';

// NextAuth v4 ships this provider as CommonJS. Resolve both Node ESM and webpack interop shapes
// explicitly so Next 15 can evaluate route handlers during the production build.
...
export const authOptions = {
  ...
  callbacks: {
    async jwt({ token, user }) {
      if (user) return syncTokenAccess(token, user);
      if (!token.uid) return token;

      // Roles are business authorization, not cosmetic profile data. Resolve them
      // from the source of truth on every session evaluation so a newly granted
      // ACCOUNTANT role can immediately receive and decide finance approvals.
      const persisted = await prisma.user.findUnique({
        where: { id: token.uid },
        select: { id: true, email: true, name: true, role: true, roles: true, teamId: true, userType: true, status: true },
      });
      return syncTokenAccess(token, persisted);
    },
    async session({ session, token }) {
      return applyTokenAccessToSession(session, token);
    },
  },
};
...
export async function currentUser() {
  ...
  return session?.user && !session.user.accessDisabled ? session.user : null;
}
