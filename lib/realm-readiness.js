...
export function evaluateRealmLaunchReadiness({
  ...
  const gates = [
    ...
    },
    {
      ...
      blocking: true,
      detail: policy.mode === 'pilot'
        ? policy.cohortStrategy === 'members'
          ? `${policy.memberIds.length} nhân sự được chọn đích danh.`
          : `${policy.roles.length} vai trò trong cohort.`
        : 'Chọn “Pilot theo cohort” trước khi mời nhân sự thật.',
    },
    {
      id: 'cohort-members',
