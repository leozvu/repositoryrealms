...
import { loadFinancialOperatingIntelligence } from './financial-operating-intelligence-admin.js';
import {
...
export async function loadRealmTreasuryDashboard(db, user) {
  ...
  const canSeeFinance = roles.includes('DIRECTOR') || roles.includes('ACCOUNTANT');
  const [walletTotal, entries, approvals, keeperApprovalsRaw, finance] = await Promise.all([
    ...
    canSeeFinance ? loadFinancialOperatingIntelligence(db, user) : Promise.resolve(null),
  ]);
  ...
  return Object.freeze({
    ...dashboard,
    financialIntelligence: finance?.financialIntelligence || null,
  });
}
