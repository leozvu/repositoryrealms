...
export function buildCeoDailyBriefing({ cockpit = null, decisionQueue = null, dashboard = null, now = new Date() } = {}) {
  ...
  const livestream = dashboard?.portfolio?.livestream || {};
  ...
  if (Number(livestream.pendingReconciliation) > 0) todayItems.push(item(
    'livestream.pending_reconciliation', 'warning', Number(livestream.pendingReconciliation), '/ceo-overview?entity=egolive', ['egolive'],
  ));
