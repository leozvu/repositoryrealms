export const REALM_GENERATED_ART_VERSION = 'guildhall-fantasy-roster-v1';
export const REALM_GENERATED_ART_ROOT = '/realms/assets/generated/phase-22';
export const REALM_GENERATED_CHARACTER_ROOT = '/realms/assets/guildhall-reforged/characters';
export const REALM_GENERATED_CHARACTER_PORTRAIT_ROOT = REALM_GENERATED_CHARACTER_ROOT;
...
export const REALM_GENERATED_CHARACTER_ARCHETYPES = Object.freeze([
  Object.freeze({ race: 'High Elf', role: 'Guild Steward' }),
  Object.freeze({ race: 'Dwarf', role: 'Forge Engineer' }),
  Object.freeze({ race: 'Human', role: 'Goldkeeper' }),
  Object.freeze({ race: 'Moon Elf', role: 'Archivist' }),
  Object.freeze({ race: 'Halfling', role: 'Community Host' }),
  Object.freeze({ race: 'Half-Orc', role: 'Project Warden' }),
]);
...
export const REALM_GENERATED_PROP_BINDINGS = Object.freeze({
  'command-dais': Object.freeze({ asset: '001', scale: 1.85 }),
...
function stableRealmIdentityHash(identity) {
  const source = identity && typeof identity === 'object'
    ? identity.userId || identity.identityId || identity.id || identity.name
    : identity;
  const text = String(source || 'realm-adventurer');
  let hash = 2166136261;
...
export function realmGeneratedCharacterSlot(identity) {
  if (identity && typeof identity === 'object') {
    ...
    if (role.includes('moon elf') || role.includes('archivist') || role === 'alchemist') return 4;
    if (role.includes('high elf') || role.includes('guild steward') || role === 'questsmith') return 1;
    if (role.includes('dwarf') || role.includes('forge engineer') || role === 'realm builder') return 2;
    if (role.includes('goldkeeper')) return 3;
    if (role.includes('halfling') || role.includes('community host') || role === 'scout') return 5;
    if (role.includes('half-orc') || role.includes('project warden') || role === 'guild master') return 6;
  }
  return stableRealmIdentityHash(identity) % REALM_GENERATED_CHARACTER_COUNT + 1;
}

export function realmGeneratedCharacterArchetype(identity) {
  return REALM_GENERATED_CHARACTER_ARCHETYPES[realmGeneratedCharacterSlot(identity) - 1];
}
...
export function realmGeneratedCharacterUrl(identity, direction = 'down') {
  ...
  return `${REALM_GENERATED_CHARACTER_ROOT}/character-${slot}-${facing}.webp`;
}

export function realmGeneratedCharacterPortraitUrl(identity) {
  const slot = String(realmGeneratedCharacterSlot(identity)).padStart(2, '0');
  return `${REALM_GENERATED_CHARACTER_PORTRAIT_ROOT}/character-${slot}-down.webp`;
}
  ...
  for (let slot = 1; slot <= REALM_GENERATED_CHARACTER_COUNT; slot += 1) {
    ...
    for (const direction of REALM_GENERATED_DIRECTIONS) {
      assets.push({
        ...
        slot,
        direction,
        url: `${REALM_GENERATED_CHARACTER_ROOT}/character-${id}-${direction}.webp`,
      });
    }
  }
  return assets;
