...
import {
  ...
  normalizeCeoServiceCredentialRotation,
...
export async function listCeoRegistryEntities(db, user, { secretResolver = resolveServerSecret } = {}) {
  ...
  return {
    ...
    commands: ['entity.update', 'sso_credential.rotate', 'service_credential.rotate'],
    destructiveCommands: [],
  };
}
...
export async function updateCeoRegistryEntity(
  ...
  const updated = await db.$transaction(async (tx) => {
    ...
    const serviceRef = current.serviceCredentialRef || current.credentialRef;
    if (command.data.enabled === true && (!secretResolver(current.credentialRef) || !secretResolver(serviceRef))) {
      throw new CeoRegistryError(
        'Provision separate SSO and service secrets before enabling this entity.',
        422,
...
export async function rotateCeoRegistryServiceCredential(
  db,
  user,
  entityId,
  input,
  ...
  requireDirector(user);
  entityId = normalizeCeoRegistryEntityId(entityId);
  const command = normalizeCeoServiceCredentialRotation(input);
  if (!secretResolver(command.serviceCredentialRef)) {
    throw new CeoRegistryError(
      'The new scoped service secret reference is not provisioned.',
      422,
      'ceo_registry_service_rotation_secret_missing',
    );
  }
  const updated = await db.$transaction(async (tx) => {
    const current = await tx.ceoEntityRegistry.findUnique({ where: { id: entityId } });
    if (!current) throw missingEntity();
    if (command.serviceCredentialRef === current.credentialRef) {
      throw new CeoRegistryError('SSO and service credentials must remain separate.', 422, 'ceo_registry_credentials_not_separated');
    }
    const nextVersion = Number(current.serviceCredentialVersion || 1) + 1;
    const result = await tx.ceoEntityRegistry.updateMany({
      where: { id: entityId, recordVersion: command.expectedVersion },
      data: {
        serviceCredentialRef: command.serviceCredentialRef,
        serviceCredentialVersion: nextVersion,
        serviceRotatedAt: now,
        recordVersion: { increment: 1 },
        status: current.enabled ? 'unverified' : 'disabled',
        consecutiveErrors: 0,
        lastErrorCode: null,
    ...
    await tx.auditLog.create({ data: {
      userId: user.id,
      userName: user.name,
      action: 'ceo_registry_service_credential_rotated',
      entity: 'ceo_entity_registry',
      refId: entityId,
      detail: `serviceCredentialVersion=${nextVersion}; expectedVersion=${command.expectedVersion}`,
    } });
    return tx.ceoEntityRegistry.findUnique({ where: { id: entityId } });
  }, { isolationLevel: 'Serializable' });
  return serializeCeoRegistryEntity(updated, { secretResolver });
...
export async function prepareCeoRegistrySync(
  ...
  // Compatibility fallback is only for databases awaiting the additive CEO-8 migration.
  // Migrated registries always contain a separate serviceCredentialRef.
  const credential = secretResolver(row.serviceCredentialRef || row.credentialRef);
  if (!credential) {
