...
export async function listLeadActionProposals(db, {
  ...
  const reviews = rows.length ? await db.leozOpsProposalReview.findMany({
    where: { proposalId: { in: rows.map(row => row.id) } },
  }) : [];
  ...
  return {
    ...
    proposals: rows.map(row => serializeLeadActionProposal(row, {
      review: reviewByProposal.get(row.id) || null,
      now,
    }).proposal),
  };
}
