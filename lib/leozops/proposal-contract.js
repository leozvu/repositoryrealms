...
import { projectProposalReview } from './review-contract.js';
...
export function serializeLeadActionProposal(row, { replayed = false, review = null, now = new Date() } = {}) {
  ...
  const reviewProjection = projectProposalReview(review);
  const nextState = expired
    ? 'refresh_brief'
    : reviewProjection.status === 'accepted'
      ? 'review_recorded_no_execution'
      : reviewProjection.status === 'rejected'
        ? 'proposal_closed'
