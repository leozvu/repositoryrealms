...
// The current Lead model stores these values as strings; preserve those source
// semantics exactly. Normalize Date instances defensively so a future schema or
// adapter still hashes the exact JSON fact served on the wire.
...
export function projectLead(lead) {
  return {
    ...
    created_at: timestampOf(lead.createdAt),
    // owner_assigned is a BOOLEAN derived from presence — never the owner id/name.
    expected_close_at: timestampOf(lead.expectedClose),
