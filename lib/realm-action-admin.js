...
const ACTIONS = new Set([
  'task.transition', 'task.assign', 'lead.transition', 'lead.expected_close.update', 'lead.source.update',
  'task.comment.create', 'lead.followup.create',
]);
...
function optionalSource(value, message, code) {
  if (value === null || value === undefined || value === '') return null;
  const source = String(value).trim().replace(/\s+/g, ' ');
  if (!/^[\p{L}\p{N} _./-]{1,80}$/u.test(source)) throw new RealmOperationError(message, 400, code);
  return source;
}

function commandOf(input = {}) {
  const action = token(input.action, /^[a-z._]{5,40}$/, 'Realm action không hợp lệ.', 'realm_action_invalid');
  ...
  if (action === 'lead.expected_close.update') {
    const expectedCloseAt = optionalDay(input.expectedCloseAt, 'Ngày chốt hiện tại không hợp lệ.', 'realm_lead_expected_close_current_invalid');
    const closeAt = optionalDay(input.closeAt, 'Ngày chốt mới không hợp lệ.', 'realm_lead_expected_close_target_invalid');
    if (expectedCloseAt === closeAt) throw new RealmOperationError('Ngày chốt mới phải khác trạng thái hiện tại.', 400, 'realm_lead_expected_close_unchanged');
    const change = { expectedCloseAt, closeAt };
    return {
      ...base, ...change,
      expectedState: expectedCloseAt || 'none', nextState: closeAt || 'none', payloadHash: payloadHash(change),
    };
  }
  if (action === 'lead.source.update') {
    const expectedSource = optionalSource(input.expectedSource, 'Nguồn Lead hiện tại không hợp lệ.', 'realm_lead_source_current_invalid');
    const source = optionalSource(input.source, 'Nguồn Lead mới không hợp lệ.', 'realm_lead_source_target_invalid');
    if (expectedSource === source) throw new RealmOperationError('Nguồn Lead mới phải khác trạng thái hiện tại.', 400, 'realm_lead_source_unchanged');
    const change = { expectedSource, source };
    return {
      ...base, ...change,
      expectedState: expectedSource || 'none', nextState: source || 'none', payloadHash: payloadHash(change),
    };
  }
  ...
  const result = await db.$transaction(async (tx) => {
    const changed = await tx.task.updateMany({
      ...
      data: {
        ...
        statusSince: now.toISOString().slice(0, 10),
        workVersion: { increment: 1 },
        ...(command.nextState === 'done' ? { completedAt: now } : {}),
      },
    });
  ...
  const result = await db.$transaction(async (tx) => {
    ...
    await tx.workItemEvent.create({ data: {
      ...
      metadata: JSON.stringify({
        previousAssigneeId: command.expectedAssigneeId,
        assigneeId: command.assigneeId,
        dueDate: command.dueDate,
        priority: command.priority,
      }),
...
async function updateLeadField(db, user, command, { field, expectedKey, valueKey }) {
  if (!canWrite('leads', user)) throw new RealmOperationError('Bạn không có quyền cập nhật Lead.', 403, 'realm_lead_forbidden');
  const before = await db.lead.findUnique({ where: { id: command.entityId } });
  if (!before || !leadInScope(before, user)) throw new RealmOperationError('Không tìm thấy Lead trong phạm vi Royal Embassy của bạn.', 404, 'realm_lead_not_found');
  const expected = command[expectedKey] ?? null;
  const value = command[valueKey] ?? null;
  if ((before[field] ?? null) !== expected) {
    throw new RealmOperationError('Lead vừa được cập nhật ở nơi khác. Hãy tải lại trước khi thao tác.', 409, 'realm_action_stale');
  }
  const result = await db.$transaction(async (tx) => {
    const changed = await tx.lead.updateMany({
      where: { id: command.entityId, [field]: expected },
      data: { [field]: value },
    });
    if (changed.count !== 1) throw new RealmOperationError('Lead vừa được cập nhật ở nơi khác. Hãy tải lại.', 409, 'realm_action_stale');
    const updated = await tx.lead.findUnique({ where: { id: command.entityId } });
    ...
    await tx.auditLog.create({ data: {
      ...
      entity: 'leads',
      refId: command.entityId,
      detail: `${command.action}: compare-and-swap applied`,
    } });
    return { updated, receipt };
  });
  return {
...
function resourceFor(action) {
  ...
  if (['lead.transition', 'lead.expected_close.update', 'lead.source.update'].includes(action)) return 'leads';
...
export async function executeRealmRecordAction(db, user, input, now = new Date()) {
  ...
  try {
    ...
    if (command.action === 'lead.expected_close.update') {
      return await updateLeadField(db, user, command, { field: 'expectedClose', expectedKey: 'expectedCloseAt', valueKey: 'closeAt' });
    }
    if (command.action === 'lead.source.update') {
      return await updateLeadField(db, user, command, { field: 'source', expectedKey: 'expectedSource', valueKey: 'source' });
    }
