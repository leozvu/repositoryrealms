...
const ACTIONS = new Set(['task.transition', 'task.assign', 'task.delegate.create', 'lead.transition', 'task.comment.create', 'lead.followup.create']);
...
function commandOf(input = {}) {
  ...
  if (action === 'task.delegate.create') {
    const title = cleanText(input.title, 180);
    if (!title) throw new RealmOperationError('Tên công việc không được để trống.', 400, 'realm_delegation_title_required');
    const assigneeId = token(input.assigneeId, ID, 'Người nhận việc không hợp lệ.', 'realm_assignment_target_invalid');
    const projectId = optionalId(input.projectId, 'Dự án không hợp lệ.', 'realm_delegation_project_invalid');
    const dueDate = optionalDay(input.dueDate, 'Hạn hoàn thành không hợp lệ.', 'realm_assignment_due_invalid');
    const priority = token(input.priority || 'medium', /^(low|medium|high|urgent)$/, 'Ưu tiên không hợp lệ.', 'realm_assignment_priority_invalid');
    const note = cleanText(input.note, 2000) || null;
    const payload = { title, assigneeId, projectId, dueDate, priority, note };
    return { ...base, ...payload, expectedState: 'none', nextState: `assigned:${assigneeId}`, payloadHash: payloadHash(payload) };
  }
...
async function assignTask(db, user, command) {
  ...
  const [scope, before, target] = await Promise.all([
    resolveManagerScope(db, user),
    db.task.findUnique({ where: { id: command.entityId }, include: { assignee: { select: { id: true, teamId: true } } } }),
    db.user.findUnique({ where: { id: command.assigneeId }, select: { id: true, teamId: true, status: true, userType: true } }),
  ]);
  ...
  if (!managerTargetInScope(scope, target, user)) {
  ...
  if (!before || (!before.assigneeId ? !managerTaskInScope(scope, before, user, { allowUnassigned: true }) : !managerTaskInScope(scope, before, user))) {
...
async function createDelegatedTask(db, user, command, now) {
  if (!hasAny(user, ['PM', 'LEAD'])) {
    throw new RealmOperationError('Chỉ PM hoặc Trưởng nhóm được giao việc mới.', 403, 'realm_delegation_forbidden');
  }
  const [scope, target, project] = await Promise.all([
    resolveManagerScope(db, user),
    db.user.findUnique({ where: { id: command.assigneeId }, select: { id: true, teamId: true, status: true, userType: true } }),
    command.projectId ? db.project.findUnique({ where: { id: command.projectId }, select: { id: true } }) : null,
  ]);
  if (!target || target.status !== 'active' || target.userType !== 'employee') {
    throw new RealmOperationError('Người nhận việc không khả dụng.', 400, 'realm_assignment_target_invalid');
  }
  if (!managerTargetInScope(scope, target, user)) {
    throw new RealmOperationError('Bạn chỉ được giao việc cho nhân sự thuộc phạm vi quản lý.', 403, 'realm_assignment_target_outside_scope');
  }
  if (command.projectId && !project) throw new RealmOperationError('Không tìm thấy dự án đã chọn.', 400, 'realm_delegation_project_invalid');
  ...
  const result = await db.$transaction(async (tx) => {
    ...
    await tx.workItemEvent.create({ data: {
      ...
      toState: command.nextState,
      receiptId: receipt.id,
      metadata: JSON.stringify({ assigneeId: command.assigneeId, projectId: command.projectId, dueDate: command.dueDate }),
    } });
    await tx.auditLog.create({ data: {
      ...
      entity: 'tasks',
      refId: created.id,
      detail: `${command.action}: none -> ${command.nextState}`,
    } });
  ...
  return {
    ...
    emissions: [{ resource: 'tasks', event: 'create', before: null, updated: result.created }],
    action: {
      id: result.receipt.id,
      type: command.action,
      entityId: command.entityId,
...
function resourceFor(action) {
  if (action === 'task.transition' || action === 'task.assign' || action === 'task.delegate.create') return 'tasks';
...
export async function executeRealmRecordAction(db, user, input, now = new Date()) {
  ...
  try {
    ...
    if (command.action === 'task.delegate.create') return await createDelegatedTask(db, user, command, now);
