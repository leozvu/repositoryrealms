...
const CONTRACTS = [
  ...
  },
  {
    ...
    intent: 'create_and_assign_work_item',
    resource: 'tasks',
    surface: 'command',
    authorization: ['PM or LEAD role', 'active target employee', 'manager company or managed-team scope'],
    businessRules: ['bounded task fields', 'existing project when selected', 'manager-target scope', 'idempotent creation'],
    receipt: 'RealmActionReceipt linked to created Task and WorkItemEvent',
    audit: 'AuditLog in the creation transaction',
  },
  {
  ...
  {
    action: 'task.adjust',
    intent: 'adjust_work_assignment_in_target_entity',
    resource: 'tasks',
    surface: 'ceo-command',
    authorization: ['Director API authentication', 'active CEO entity membership', 'command.task.adjust scope'],
    businessRules: ['target audience match', 'task field allowlist', 'updatedAt CAS', 'terminal task lock', 'no workflow status bypass'],
    receipt: 'CeoEntityCommandReceipt with correlation and payload hash',
    audit: 'AuditLog in the target record transaction',
  },
  {
  ...
  },
  {
    ...
    intent: 'request_project_status_from_target_entity',
    resource: 'tasks',
    surface: 'ceo-command',
    authorization: ['Director API authentication', 'active CEO entity membership', 'command.project.status.request scope'],
    businessRules: ['target audience match', 'existing project', 'active employee target', 'creates a request task instead of mutating project status'],
    receipt: 'CeoEntityCommandReceipt linked to local Task',
    audit: 'AuditLog in the target record transaction',
  },
  {
  ...
  },
  {
    ...
    intent: 'request_bounded_work_from_another_group_company',
    resource: 'approvals',
    surface: 'ceo-group-workforce',
    authorization: ['Director API authentication', 'active CEO entity membership', 'command.group-workforce.request scope'],
    businessRules: ['employing entity owns approval', 'employee directory consent', 'bounded assignment dates and capacity', 'no payroll or employment transfer'],
    receipt: 'CeoEntityCommandReceipt linked to the employing entity Approval',
    audit: 'AuditLog in the target approval transaction',
  },
  {
    action: 'incident.acknowledge',
    intent: 'acknowledge_registered_incident',
    resource: 'tickets',
    surface: 'ceo-command',
    authorization: ['Director API authentication', 'active CEO entity membership', 'command.incident.acknowledge scope'],
    businessRules: ['target audience match', 'incident registry source only', 'open-state and updatedAt CAS', 'no incident payload copied to Portal'],
    receipt: 'CeoEntityCommandReceipt linked to local Ticket',
    audit: 'Payload-free AuditLog in the target ticket transaction',
  },
  {
    action: 'message.send',
