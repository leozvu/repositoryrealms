...
async function readGoldSettings(db = prisma) {
...
async function autoGoldEarnedToday(userId, today, db = prisma) {
  const tomorrow = new Date(`${today}T00:00:00.000Z`);
  tomorrow.setUTCDate(tomorrow.getUTCDate() + 1);
  const rows = await db.realmGoldEntry.findMany({
    where: {
      userId,
      sourceType: { in: ['ontime_task', 'full_attendance_day'] },
      createdAt: { gte: new Date(`${today}T00:00:00.000Z`), lt: tomorrow },
    },
    select: { amount: true },
  });
...
async function writeGoldEntry({ userId, amount, source, sourceId, label, today, now }, db = prisma) {
  const key = goldIdempotencyKey(userId, source, sourceId);
  const existing = await db.realmGoldEntry.findUnique({ where: { idempotencyKey: key } });
  if (existing) return { skipped: 'duplicate' };

  const earnedToday = await autoGoldEarnedToday(userId, today, db);
  const allowed = capDailyGold(amount, earnedToday, (await readGoldSettings(db)).dailyCap);
  if (allowed <= 0) return { skipped: 'daily_cap' };

  try {
    await db.realmGoldEntry.create({
      data: {
        userId,
        type: 'quest_reward',
        amount: allowed,
...
export async function awardGoldForTask(task, previous, { now = new Date(), db = prisma } = {}) {
  const settings = await readGoldSettings(db);
...
export async function awardGoldForAttendance(attendance, { now = new Date(), db = prisma } = {}) {
  const settings = await readGoldSettings(db);
...
export async function goldWalletOf(userId, { db = prisma } = {}) {
  const rows = await db.realmGoldEntry.findMany({ where: { userId }, select: { amount: true, renown: true } });
  return rows.reduce(
