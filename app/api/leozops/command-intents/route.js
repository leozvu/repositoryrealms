...
import { handleLeozOpsCommandInbox } from '@/lib/leozops/command-handler';

export const runtime = 'nodejs';
export const dynamic = 'force-dynamic';

const loadLeads = () => prisma.lead.findMany({
  select: { id: true, source: true, value: true, stage: true, ownerId: true, createdAt: true, expectedClose: true },
});

async function run(req) {
  if (process.env.LEOZOPS_COMMAND_ENABLED !== 'true') return NextResponse.json({ error: 'not found' }, { status: 404 });
  const user = await currentUser();
  const result = await handleLeozOpsCommandInbox(req, {
    env: process.env, db: prisma, getUser: async () => user, loadLeads,
  });
