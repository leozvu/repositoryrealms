...
import { loadRealmEconomyDashboard } from '@/lib/realm-economy-admin';
...
function integrationDisabled(trace) {
  return realmJsonResponse(trace, {
    error: 'Gold Economy Observatory chưa được kết nối ERP.',
...
async function authorizedUser() {
  ...
  if (isFreelancer(user)) throw new RealmOperationError('Freelancer không có quyền xem Gold Economy Observatory.', 403, 'freelancer_forbidden');
  return user;
}

function errorResponse(trace, error) {
  return realmErrorResponse(trace, error, { fallbackMessage: 'Không thể tải Gold Economy Observatory.', fallbackCode: 'realm_economy_error' });
}

export async function GET(request) {
  const trace = startRealmApiRequest(request, { route: 'realm.economy', operation: 'dashboard.read' });
  if (process.env.REALM_ERP_SYNC_ENABLED !== '1') return integrationDisabled(trace);
  try {
    const user = await authorizedUser();
    const access = realmSurfaceDecision(user, 'economy', await loadRealmCompanyModules(prisma));
    if (!access.allowed) throw new RealmOperationError(access.reason, 403, access.code);
    return realmJsonResponse(trace, await loadRealmEconomyDashboard(prisma, user), { code: 'realm_economy_ready' });
  } catch (error) {
