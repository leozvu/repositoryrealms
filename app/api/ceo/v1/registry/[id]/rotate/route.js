...
import { rotateCeoRegistryCredential } from '@/lib/ceo-entity-registry-admin';
...
export async function POST(request, { params }) {
  ...
  try {
    ...
    const entity = await rotateCeoRegistryCredential(prisma, user, params.id, body);
    ...
    console.error('ceo_registry_rotation_failed', error);
    return NextResponse.json(
      { error: 'CEO entity credential rotation failed', code: 'ceo_registry_rotation_failed' },
      { status: 503, headers },
    );
  }
}
