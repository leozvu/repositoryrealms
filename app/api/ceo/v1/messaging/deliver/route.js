...
export async function POST(request) {
  ...
  try {
    ...
    return NextResponse.json({ contract: result.contract, version: result.version, receipt: result.receipt, repository: result.repository }, { status: result.idempotent ? 200 : 201, headers });
  } catch (error) {
    ...
    return NextResponse.json({ error: known ? error.message : 'Message delivery is unavailable.', code: known ? error.code : 'ceo_messaging_target_unavailable' }, { status: known ? error.status : 503, headers });
  }
}
