...
import { useResource, ResourceError, Icon, ConfirmDialog, Badge, Forbidden, useToast } from '@/components/ui';
...
export default function QuotesPage() {
  ...
  const { rows, loading, error, forbidden, mutating, remove, refresh } = useResource('quotes');
  ...
  const busy = mutating || converting;
  ...
  const convert = async (quote, target) => {
    if (conversionRef.current || mutating) return null;
    ...
    try {
      ...
      if (!body?.id || body._blocked) {
        toast(body?._notice || 'Chưa xác minh được kết quả. Hãy kiểm tra danh sách hóa đơn hoặc dự án trước khi thử lại.', 'error');
        return null;
      }
      await Promise.allSettled([invoices.refresh(), projects.refresh(), refresh()]);
      return body;
    } catch {
      toast('Mất kết nối; chưa xác minh được kết quả chuyển báo giá. Hãy kiểm tra danh sách hóa đơn hoặc dự án trước khi thử lại.', 'error');
      return null;
    } finally {
  ...
  const columns = useMemo(() => [
    ...
    { id: 'client', header: 'Khách hàng', size: 220, cell: ({ row }) => client(row.original.clientId)?.name || (clientUnavailable ? 'Chưa tải được khách hàng' : 'Không xác định'), meta: { exportValue: (row) => client(row.clientId)?.name || '' } },
      ...
      cell: ({ row }) => {
        ...
        return (
          <div className="row-actions">
            {canWrite && <button className="icon-btn" disabled={busy} title="Chuyển thành hóa đơn" onClick={() => setModal({ mode: 'toinv', row: quote })}><Icon name="invoices" size={16} /></button>}
            {canWrite && quote.status === 'accepted' && <button className="icon-btn" disabled={busy} title="Tạo dự án" onClick={() => setModal({ mode: 'toproj', row: quote })}><Icon name="projects" size={16} /></button>}
            {canWrite && <button className="icon-btn" disabled={busy || !!clientUnavailable} title="Gửi email" onClick={() => setModal({ mode: 'email', row: quote })}><Icon name="mail" size={16} /></button>}
            <button className="icon-btn" disabled={busy || !!clientUnavailable} title="In hoặc xuất PDF" onClick={() => printDoc(quote, 'quote', client(quote.clientId)?.name || '', client(quote.clientId) || {})}><Icon name="print" size={16} /></button>
            {canWrite && <button className="icon-btn" disabled={busy} title="Mở trình soạn" onClick={() => router.push(`/quotes/${quote.id}`)}><Icon name="edit" size={16} /></button>}
            {canWrite && <button className="icon-btn danger" disabled={busy} title="Xóa" onClick={() => setModal({ mode: 'del', row: quote })}><Icon name="trash" size={16} /></button>}
          </div>
        );
      },
    },
  ], [canWrite, clients.rows, projects.rows, router, busy, clientUnavailable]);
  ...
  return (
    <div className="record-index-page">
      <PageHeader
        ...
        actions={canWrite && <button className="btn btn-primary" disabled={busy || !!clientUnavailable} onClick={() => {
      ...
      <ResourceError error={invoices.error && `Hóa đơn: ${invoices.error}`} onRetry={invoices.refresh} loading={invoices.loading} />
      ...
      {loading && <div role="status" aria-live="polite">Đang tải báo giá…</div>}
      {!error && (!loading || rows.length > 0) && <DataTable
        data={filtered}
        columns={columns}
        storageKey="quotes"
        ...
        actions={canWrite && <button className="btn btn-primary btn-sm" disabled={busy || !!clientUnavailable || !clients.rows.length} onClick={() => router.push('/quotes/new')}><Icon name="plus" size={15} />Tạo báo giá</button>}
        renderMobile={(quote) => (
          <article className="mobile-record" key={quote.id}>
            <div><Badge map="quote" k={quote.status} /><time>{fmtDate(quote.date)}</time></div>
            <strong>{quote.code} · {client(quote.clientId)?.name || (clientUnavailable ? 'Chưa tải được khách hàng' : 'Không xác định')}</strong>
      ...
      {modal?.mode === 'del' && <ConfirmDialog msg={`Xóa báo giá ${modal.row.code}?`} onClose={() => setModal(null)} onYes={async () => { const result = await remove(modal.row.id); if (!result) return false; toast('Đã xóa báo giá'); return true; }} />}
