...
async function main() {
  ...
  await prisma.lead.createMany({
    data: [
      { name: 'Chị Quỳnh — chuỗi trà sữa MoMo Tea', company: 'MoMo Tea', email: 'quynh@momotea.vn', phone: '0905123123', source: 'Facebook', value: 60000000, stage: 'new', ownerId: am.id, createdAt: D(-3), note: 'Muốn chạy ads khai trương 3 chi nhánh' },
      { name: 'Anh Dũng — Gym Titan', company: 'Titan Fitness', email: 'dung@titanfit.vn', phone: '0918777666', source: 'Website', value: 45000000, stage: 'contacted', ownerId: am.id, createdAt: D(-8), note: 'Đã gọi lần 1, hẹn demo tuần sau' },
      { name: 'Chị Lan — Tiệm bánh Mây', company: 'Mây Bakery', email: 'lan@maybakery.vn', phone: '0902555444', source: 'Giới thiệu', value: 90000000, stage: 'proposal', ownerId: am.id, createdAt: D(-20), note: 'Đã gửi đề xuất branding + social, chờ phản hồi' },
      { name: 'Anh Sơn — Nội thất SGHome', company: 'SGHome', email: 'son@sghome.vn', phone: '0938666777', source: 'Giới thiệu', value: 150000000, stage: 'negotiation', ownerId: am.id, createdAt: D(-12), note: 'Đang thương lượng phạm vi TVC + KOL' },
  ...
  const p1 = await prisma.project.create({ data: { name: 'Chiến dịch BST Thu Đông — EVA', clientId: eva.id, service: 'Ads + KOL + Video', budget: 180000000, status: 'active', startDate: D(-40), deadline: D(+35), progress: 55 } });
  const p2 = await prisma.project.create({ data: { name: 'Retainer Fanpage & Ads — Cà phê Nhà Mình', clientId: cafe.id, service: 'Quản lý Fanpage + Ads', budget: 90000000, status: 'active', startDate: D(-90), deadline: D(+90), progress: 45 } });
  const p3 = await prisma.project.create({ data: { name: 'Website & Landing page — Smile Plus', clientId: smile.id, service: 'Landing page', budget: 60000000, status: 'active', startDate: D(-60), deadline: D(-5), progress: 80 } });
